...
//!
//! CPI helpers for delegating / undelegating PDAs to the MagicBlock
//! delegation program.
...
use super::{
    ...
    utils::{close_pda, close_pda_with_system_transfer, create_pda, seeds_with_bump},
};
...
pub fn delegate_account<'a, 'info>(
    ...
    let new_data = accounts.pda.try_borrow_data()?.to_vec();
...
/// Finalize undelegation on base layer after the ER has called
/// `commit_and_undelegate_accounts`. Re-creates the PDA owned by the original
/// program and copies the buffered state back into it.
pub fn undelegate_account<'a, 'info>(
    ...
    let account_signer_seeds_with_bump: &[&[&[u8]]] = &[&*seeds_with_bump(
    ...
    create_pda(
        ...
        owner_program,
        buffer.data_len(),
        account_signer_seeds_with_bump,
        system_program,
        payer,
    )?;
